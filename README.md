...
一个面向 Codex 的中文民间故事手机伪纪录图文技能。它把传说、怪谈、志怪、地方口述或用户提供的灵感，整理成 24–27 张 4:5 图文轮播，并通过故事、专业分镜、参考图三次主创作确认，配合结构化自审、一次返修上限和发布校验完成交付。
...
- 第一阶段只交付可直接编辑的故事脚本，确认后再规划分镜。
- 同时维护清晰的五列专业分镜和完整的 AI 生产分镜，两者用途分离。
- 在故事、专业分镜和角色/地点母版后分别等待用户明确确认。
...
故事、专业分镜、参考图和返修报告批准是不可绕过的内容门槛。批准文件使用 SHA-256 固化，后续编辑会使下游批准失效。传输恢复、参考图派生、批次切换和单图跳过不会额外要求用户确认。
...
技能会先只输出故事脚本并等待确认；随后生成清晰的专业分镜和内部生产资料并再次等待确认；参考图确认后才逐镜生成、检查、返修和打包。
