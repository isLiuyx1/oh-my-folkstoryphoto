...
一个面向 Codex 的中文民间故事手机或旧 DV 伪纪录图文技能。它把传说、怪谈、志怪、地方口述或用户灵感整理成 30–39 张 4:5 长篇图文轮播。v6 在真实采集、三张校准和用户审批之外，新增了针对伪纪录故事与逐图字幕的去公式化写作规则。

核心原则：每张图首先要像一个真实的人在当时条件下自然拍下并保留的私人照片，其次才承担悬疑、怪谈或奇观叙事。真实感来自拍摄者、拍摄原因、受限机位、人物行为和设备能力，而不是统一噪点、假日期戳、复古滤镜或电影化调色。

## v6.0 更新

- 写作前建立“声音卡”，限定人物职业、说话习惯、当时知识与术语边界。
- 字幕改用“连续三图测试”，允许短句、停顿、改口和前图代词；取消8字硬下限，保留非空与48字上限。
- 强调的数字、物件和异常必须在后文发生作用；技术词必须有合理知识来源。
- 新增 `text_audit.py`，区分硬结构错误与需人工判断的AI腔警告，不自动改写。
- 新增已完成项目的文字专修事务：备份、提交、用户批准或回滚，全程不触碰图片、参考资产和manifest。
- 写作原则借鉴 [Humanizer-zh](https://github.com/op7418/Humanizer-zh) 的 MIT 许可项目；详见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。

v5 的真实性方案、采集设备档案、三张校准、机位物理、结构化逐图审查和单轮返修门槛全部保留。
...
- 发布字幕服从人物声音和当时知识，通过连续三图、朗读和删字幕测试，避免空洞悬念和公式化解释。
...
插件和技能还应使用当前 Codex 安装随附的官方校验器验证。当前回归套件覆盖 schema v1–v6、传输恢复、目录治理、字幕审查、文字专修、真实性门槛及发布打包；CI 还会运行 Python 语法检查和 JSON 解析检查。
