...
- 每个失败项最多自动修复一次；复审仍失败时转交用户。
- 将网络错误与内容失败分离，持久化请求指纹、重试次数和熔断状态。
...
`awaiting_plan_approval` 和 `awaiting_reference_approval` 是不可绕过的人工门槛。`Stop` hook 只会提醒 Codex 继续未完成的审查，不会替用户批准方案，也不会增加第二轮返修。
...
- 仓库不包含商业 IP、影视角色或实践项目图片，只保留匿名化规则与测试样例。
