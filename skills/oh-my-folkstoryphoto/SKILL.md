...
description: Create and organize Chinese folk-story, legend, mystery, horror, wonder, phone mockumentary, old-DV found-footage, AI photorealistic, Douyin or Xiaohongshu photo-carousels. Use when Codex needs source adaptation, a mandatory user-approved capture-realism direction, story-first planning, 30–39-frame evidence chains, character/prop continuity, three-shot realism calibration, built-in image generation, structured hard-failure review, one approved repair round, and release packaging.
---
...
- 新项目使用 `workspace_layout.py init-project` 创建schema v5独立目录，不覆盖同名项目；旧schema不自动迁移。
- 保留五个停止门槛：真实性方向、故事、专业分镜、参考图、三张真实性校准。每次必须收到明确批准并保存文件哈希；沉默、含糊肯定和“先看看”不算批准。
- 返修报告另需明确批准。先完成并审查全部原图，再统一返修；每图最多一次内容返修，返修后仍有硬失败进入 `needs_user`。
...
- [output-spec.md](references/output-spec.md)：schema v5目录、字段和批准哈希。
...
- 根目录五列专业分镜；
- 发布文件说明；
- 制作资料中的八段式创作方案、schema v5 AI分镜、视觉设定、短提示词和参考说明。
...
每张生成后查看原尺寸与联系表，写入 `08-系统文件/03-真实性审查/NN.json`，再运行：
...
首版失败用 `queue-repair`登记，不立即返修。全部原图审查完成后运行 `prepare-repair-report`并停止；用户批准指定图号后运行 `authorize-repairs --user-approved`，再统一返修。

## 7. 全篇验收与发布

按故事顺序和采集配置分别制作联系表，检查是否像同几台普通设备产生的文件。验证真实性批准、三张校准、普通相册测试、设备连续性、身份与道具、证据链、字幕、场景原生文字和最终余波。

全部图片为 `pass` 后创建manifest，运行状态验证与 `package_release.py`。完成自审记录、验收记录和AI声明后进入 `complete`。
