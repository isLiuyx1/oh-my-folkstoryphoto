...
- 任何派生参考都必须先核对项目的 `参考图说明.md`、文件名和本镜出镜表；不得仅凭外观猜测人物身份。预检为每张输入增加同序 `--reference-role`，写清人物姓名和锚定内容；角色数量或身份不一致时不得调用生图。
